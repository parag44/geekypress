	...
	<?php if ( ! empty( $interests ) && is_array( $interests ) ) : ?>
		<div class="wp-block-group terminal-list">
			<?php foreach ( $interests as $item ) : ?>
				<?php
				$icon = ! empty( $item['icon'] ) ? $item['icon'] : 'globe';
				?>
				<p class="content-text">
					<span class="terminal-list-badge" aria-hidden="true">
						<?php echo geekypress_get_icon( $icon, 'interest-badge-icon', 20 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</span>
