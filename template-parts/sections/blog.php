...
$slider_enabled = (bool) get_theme_mod( 'geekypress_blog_slider_enabled', true );
$per_screen     = absint( get_theme_mod( 'geekypress_blog_per_screen', 3 ) );
if ( $per_screen < 1 || $per_screen > 4 ) {
	$per_screen = 3;
}
$posts_per_page = absint( get_theme_mod( 'geekypress_blog_count', 9 ) );
if ( $posts_per_page < 1 ) {
	$posts_per_page = 9;
}
if ( $slider_enabled && $posts_per_page < ( $per_screen * 2 ) ) {
	$posts_per_page = max( 6, $per_screen * 2 );
}
...
<section id="blog" class="wp-block-group alignwide terminal-section terminal-blog<?php echo $slider_enabled ? ' has-blog-slider' : ''; ?>">
	<div class="terminal-section-header terminal-blog-header">
		<div>
			...
			<?php endif; ?>
			<?php if ( ! empty( $title ) ) : ?>
				<h2 class="section-title geekypress-preview-blog-title"><?php echo esc_html( $title ); ?><span class="terminal-title-cursor" aria-hidden="true">_</span></h2>
			<?php endif; ?>
		</div>

		<div class="terminal-blog-header-actions">
			...
			<?php if ( $slider_enabled ) : ?>
				<div class="terminal-blog-slider-nav" role="group" aria-label="<?php esc_attr_e( 'Blog carousel navigation', 'geekypress' ); ?>">
					<button type="button" class="terminal-blog-nav-btn terminal-blog-prev" aria-label="<?php esc_attr_e( 'Previous articles', 'geekypress' ); ?>" title="<?php esc_attr_e( 'Previous', 'geekypress' ); ?>">
						<span class="gp-nav-char" aria-hidden="true">&lt;</span>
					</button>
					<button type="button" class="terminal-blog-nav-btn terminal-blog-next" aria-label="<?php esc_attr_e( 'Next articles', 'geekypress' ); ?>" title="<?php esc_attr_e( 'Next', 'geekypress' ); ?>">
						<span class="gp-nav-char" aria-hidden="true">&gt;</span>
					</button>
				</div>
			<?php endif; ?>
	...
	<?php if ( $slider_enabled ) : ?>
	<div class="terminal-blog-slider-wrap" data-per-screen="<?php echo esc_attr( $per_screen ); ?>" style="--per-screen: <?php echo esc_attr( $per_screen ); ?>;">
		<div class="terminal-blog-slider-track">
	<?php else : ?>
	...
	<?php endif; ?>
		...
		if ( $blog_query->have_posts() ) :
			...
			$placeholders = array(
				...
				array(
					'title'    => 'Integrating Local LLM Workflows into Modern Web Applications',
					'category' => 'AI_Workflows',
					'date'     => '2026-05-22',
					'read'     => '7 min read',
					'excerpt'  => 'Pragmatic developer patterns for streaming embeddings, semantic indexing, and local inference models without third-party latency.',
				),
				array(
					'title'    => 'High-Performance SQL Indexing and Caching Patterns in MySQL',
					'category' => 'Database',
					'date'     => '2026-04-18',
					'read'     => '4 min read',
					'excerpt'  => 'Deep dive into composite B-Tree indexes, query execution execution plans, and persistent object caching strategies.',
				),
				array(
					'title'    => 'Hardening Web Application Security: Nonces, Sanitization & CSP',
					'category' => 'Security',
					'date'     => '2026-03-30',
					'read'     => '5 min read',
					'excerpt'  => 'Essential defense-in-depth principles for neutralizing injection vectors, securing REST endpoints, and enforcing strict CSP headers.',
				),
			);

			$render_count = $slider_enabled ? count( $placeholders ) : min( $posts_per_page, count( $placeholders ) );
